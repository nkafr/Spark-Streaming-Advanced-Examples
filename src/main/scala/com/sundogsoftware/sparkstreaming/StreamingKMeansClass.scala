...
//import com.sundogsoftware.sparkstreaming.StreamingKMeans
 ...
 *  5 clusters.
 */
object StreamingKMeansClass {
  
  def main(args: Array[String]) {

    // Create the context with a 1 second batch size
    val ssc = new StreamingContext("local[*]", "StreamingKMeansClass", Seconds(1))
    
    setupLogging()
    
    
    /*
     *val trainingData = ssc.textFileStream("/home/nikos/train").map(Vectors.parse).cache()
      val testData = ssc.textFileStream("/home/nikos/test").map(LabeledPoint.parse) 
    
    df=read.csv("tocluster.csv",header=FALSE)
    km = kmeans(df, centers = 5, iter.max = 1000)
    km 
