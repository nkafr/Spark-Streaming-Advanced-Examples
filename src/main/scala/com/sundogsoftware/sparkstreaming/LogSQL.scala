...
object LogSQL {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("LogSQL").setMaster("local[*]").set("spark.sql.warehouse.dir", "/tmp")
    ...
    requests.foreachRDD((rdd, time) => {
      ...
      wordCountsDataFrame.show()
        ...
        //Write the output dataframes to json files
        
         requestsDataFrame.write.json("/tmp/"+time)
...
object SQLContextSingleton {
  ...
  @transient  private var instance: SQLContext = _         //underscore means initialization to the default value which in thid case is null
  //equivalent to @transient  private var instance: SQLContext = null    for more examples see http://stackoverflow.com/questions/8000903/what-are-all-the-uses-of-an-underscore-in-scala
